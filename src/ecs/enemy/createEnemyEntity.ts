...
export function createEnemyEntity(options: CreateEnemyEntityOptions): EntityId {
  ...
  options.world.addComponent(entityId, EnemyUiPresentationComponent, {
    linkOffsetY: options.config.uiLinkOffsetY,
    baseScale: options.config.uiBaseScale,
    maxVisibleDistance: options.config.uiMaxVisibleDistance,
    damageRevealDuration: options.config.uiDamageRevealDuration,
  });
...
function createEnemyRagdoll(
  ...
  const ragdoll = new Ragdoll(skeleton, armatureNode, [
    { bones: ['mixamorig7:Hips'], size: 0.42, boxOffset: 0.01 },
    {
